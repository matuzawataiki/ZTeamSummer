...
void SoundManager::PlayBGM(const SoundKind kind)
{
	// BGMが生成されていない
	if (m_bgm == nullptr) {
		// 生成
	...
	// 初期化
	m_bgm->Init(static_cast<int>(kind));
...
SoundHandle SoundManager::PlaySE(const SoundKind kind, const bool isLood, const bool is3D)
{
	// ハンドルが最大数になったら使えない
	...
	se->Init(static_cast<int>(kind), is3D);
	se->Play(isLood);
