#include "stdafx.h"
#include "CharactorColliderComponent.h"
...
void CharactorColliderComponent::Init()
{
	...
	rigidBody->GetRigidBody().GetBody()->setUserIndex(enCollisionAttr_Character);
	rigidBody->GetRigidBody().GetBody()->setCollisionFlags(btCollisionObject::CF_CHARACTER_OBJECT);

	btTransform& trans = rigidBody->GetRigidBody().GetBody()->getWorldTransform();
	//剛体の位置を更新。
...
void CharactorColliderComponent::Update()
{
	...
	//XZ平面での衝突検出と衝突解決を行う。
	{
		...
		while (true) {
			...
			callback.me = rigidBody->GetRigidBody().GetBody();
			callback.startPos = posTmp;
			//衝突検出。
			PhysicsWorld::GetInstance()->ConvexSweepTest((const btConvexShape*)collider->GetGhostObject().GetCollider()->GetBody(), start, end, callback);
			...
			if (callback.isHit) {
				...
				Vector3 currentDir;
				currentDir.Subtract(nextPosition, position);
				currentDir.y = 0.0f;
	...
	//下方向を調べる。
	{
		...
		btRigidBody* btBody = rigidBody->GetRigidBody().GetBody();
		//剛体を動かす。
